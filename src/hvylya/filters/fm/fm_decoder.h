...
{
  public:
    typedef typename FilterBaseType<FmDecoder>::Type Base;

    FmDecoder(uint32_t sample_rate);
  ...
  private:
    ...
    typedef core::SimdVector<float, core::NonAligned> ScalarVector;

    std::size_t sample_rate_;
    //core::RunningSum<T> dc_;
    //double total_sum_;
